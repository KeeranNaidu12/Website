...
export default function Technology({name,image,description,date,github,link, tech} : ProjectProps){
    return (
      <div className="flex flex-col rounded-2xl bg-white/5 border border-white/10 p-5 sm:p-7 md:p-10 gap-4 sm:gap-5 md:gap-6 transition-all duration-300 hover:border-sky-400/60">
        ...
        <ul className='flex flex-wrap gap-2'>
          {tech.map((tech => 
            <li
              key={tech}
              className="rounded-full bg-sky-500/20 text-sky-300 text-sm sm:text-base md:text-lg font-bold px-3 sm:px-4 py-1"
            >
              {tech}
            </li>
          ))}
